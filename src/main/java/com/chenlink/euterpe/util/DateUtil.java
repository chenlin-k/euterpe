...
//			System.out.println(dd.substring(15));
     ...
     * @return
     */
    public static String StringMonthAddYear(String month){
        SimpleDateFormat df = new SimpleDateFormat("yyyyMM");
        int year= Integer.parseInt(df.format(new Date()).substring(0,4));
        int mm= Integer.parseInt(df.format(new Date()).substring(4,6));
        int mo= Integer.parseInt(month.substring(0,month.indexOf("月")));
        if(mm-mo<0){
            year= year-1;
        }
        if (mo<10){
