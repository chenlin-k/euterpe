 ...
 */
@Component
public class CmmailImpl implements EmailTemplate {

    private static final Logger logger = Logger.getLogger(CmmailImpl.class);

    @Autowired
    private BillCycleMapper bm;

    @Override
    public JSONObject mailAnaly(String[] sts, String idcard) {
        return null;
    }
    ...
    public JSONObject mailAnalys(String[] sts,String param)throws Exception {
        ...
        JSONObject jsonstring =JSONObject.fromObject(param);
        JSONArray ja=JSONArray.fromObject(jsonstring.get("data"));
        String payments = null;//本期应还款额
        String creditLine = null;//信用额度
        String paymentsDate = null;//还款日
        String creditNumber = null;//信用卡卡号
        String html=null;//网页html内容
        String sendMail=null;//发送人
        String subject=null;//主题
        for (int i=0;i<=sts.length-1;i++) {
            try {
            html=sts[i];
            Document doc = Jsoup.parse(html);
            sendMail=ja.getJSONObject(i).get("sendMail").toString();
            subject=ja.getJSONObject(i).get("subject").toString();
            bcie.setIdCard(ja.getJSONObject(i).get("idCard").toString());
            bcie.setSenderUrl(sendMail);
            bcie.setReceiveAddUrl(ja.getJSONObject(i).get("receiveMail").toString());
            bcie.setSendTime(ja.getJSONObject(i).get("sendTime").toString());
            bcie.setSubject(subject);
            bcie.setInfoSource("139邮箱");
            //1.中信银行邮件解析citic
            if (/*sendMail.equals("<1479055311@qq.com>") && */subject.contains("中信银行信用卡电子账单")) {
                ...
                creditNumber = doc.select("span[id=fixBand5]").select("font").get(12).text();
                if (!StringUtils.isEmpty(payments) && !StringUtils.isEmpty(creditLine) && !StringUtils.isEmpty(paymentsDate) && !StringUtils.isEmpty(creditNumber)) {
                    ...
                    bcie.setErrorInfo("请注意,可能是139邮箱的新类型账单，获取账单失败---<中信银行信用卡>");
                }
                saveBillCycleInfoEntity(bcie);
                listbills.add(i,bcie);
                continue;
            }
            //2.交通银行邮件解析boConm
            else if (/*sendMail.equals("<pccc@bocomcc.com>") &&*/ subject.contains("交通银行信用卡电子账单")) {
                Elements rows = doc.select("table[id=table1]").select("td");
                payments = rows.get(2).text();
                creditLine = rows.get(8).text();
                paymentsDate = rows.get(0).text();
                creditNumber = doc.select("div#container").select("p").get(2).text();
                if (!StringUtils.isEmpty(payments) && !StringUtils.isEmpty(creditLine) && !StringUtils.isEmpty(paymentsDate) && !StringUtils.isEmpty(creditNumber)) {
                    ...
                    bcie.setPaymentsDate(paymentsDate);
                    bcie.setCreditLine(creditLine);
                    creditNumber = creditNumber.substring(creditNumber.length() - 16, creditNumber.length());
                    bcie.setCreditNumber(creditNumber);
                    bcie.setIsSuccess(0);
                } else {
                    bcie.setIsSuccess(1);
                    bcie.setErrorContent("邮件保存的路径为：" + StringUtil.saveAsFileWriter(html));
                    bcie.setErrorInfo("请注意,可能是139邮箱新类型账单，获取账单失败---<交通银行信用卡>");
                }
                saveBillCycleInfoEntity(bcie);
                listbills.add(bcie);
                continue;
            ...
            //4.光大银行Ceb   注意(由于邮件是转发的邮件，并非原件，因此可能在获取原件时会有所出入)
                ...
                Elements es = doc.select("table").get(6).getElementsByTag("td");
                payments = es.get(14).text();
                paymentsDate = es.get(12).text();
                creditLine = es.get(13).text();
                creditNumber = doc.select("table").get(14).getElementsByTag("td").get(10).text();
                if (!StringUtils.isEmpty(payments) && !StringUtils.isEmpty(creditLine) && !StringUtils.isEmpty(paymentsDate) && !StringUtils.isEmpty(creditNumber)) {
                    if (payments.contains("￥") && creditLine.contains("￥")) {
                        payments = payments.replaceAll("[a-zA-Z￥,]", "").trim();
                        creditLine = creditLine.replaceAll("[a-zA-Z￥,]", "").trim();
                    ...
                    bcie.setIsSuccess(1);
                    bcie.setErrorContent("邮件保存的路径为：" + StringUtil.saveAsFileWriter(html));
                    bcie.setErrorInfo("请注意,可能是139邮箱的新类型账单，获取账单失败---<光大银行信用卡>");
                }
                saveBillCycleInfoEntity(bcie);
                listbills.add(bcie);
                continue;
            ...
            //5.招商银行cmb
                ...
                if (!StringUtils.isEmpty(payments) && !StringUtils.isEmpty(paymentsDate)) {
                    ...
                    bcie.setIsSuccess(1);
                    bcie.setErrorContent("邮件保存的路径为：" + StringUtil.saveAsFileWriter(html));
                    bcie.setErrorInfo("请注意,可能是139邮箱新类型账单，获取账单失败---<招商银行信用卡>");
                }
                saveBillCycleInfoEntity(bcie);
                listbills.add(bcie);
                continue;
            ...
            //6.建设银行ccb
                ...
                Elements rows = doc.select("table").get(8).select("td");
                payments = rows.get(10).text();
                paymentsDate = rows.get(4).text();
                creditLine = doc.select("table").get(7).select("td").get(4).select("font").get(0).text();
                creditNumber = doc.select("table").get(15).select("td").get(7).text();
                if (!StringUtils.isEmpty(payments) && !StringUtils.isEmpty(creditLine) && !StringUtils.isEmpty(paymentsDate) && !StringUtils.isEmpty(creditNumber)) {
            ...
            //7.广发银行Cgb
                ...
                if (!StringUtils.isEmpty(payments) && !StringUtils.isEmpty(creditLine) && !StringUtils.isEmpty(paymentsDate) && !StringUtils.isEmpty(creditNumber)) {
                    ...
                    bcie.setIsSuccess(1);
                    bcie.setErrorContent("邮件保存的路径为：" + StringUtil.saveAsFileWriter(html));
                    bcie.setErrorInfo("请注意,可能是139邮箱的新类型账单，获取账单失败---<广发银行信用卡>");
                }
                saveBillCycleInfoEntity(bcie);
                listbills.add(bcie);
                continue;
            ...
            //8.江西银行Jxbank
                ...
                if (!StringUtils.isEmpty(payments) && !StringUtils.isEmpty(creditLine) && !StringUtils.isEmpty(paymentsDate) && !StringUtils.isEmpty(creditNumber)) {
                    if (payments.contains("￥")) {
                        payments = payments.replaceAll("[\\u4e00-\\u9fa5a-zA-Z￥,,]", "");
                        payments = payments.replace((char) 12288, '\0').trim();
                    }
                    creditLine = creditLine.replaceAll("[\\u4e00-\\u9fa5,]", "");
                    creditLine = creditLine.replace((char) 12288, '\0').trim();
                    paymentsDate = DateUtil.getDateformat(paymentsDate);
                    bcie.setBankName("jxbank");
                    bcie.setPayments(payments);
                    paymentsDate = DateUtil.getDateformat(paymentsDate).replaceAll("[\\u4e00-\\u9fa5,：]", "");
                    paymentsDate = paymentsDate.replace((char) 12288, '\0');//去除12288汉字空格
                    paymentsDate = paymentsDate.trim();
                    bcie.setPaymentsDate(paymentsDate);
                    bcie.setCreditLine(creditLine);
                    ...
                    bcie.setIsSuccess(1);
                    bcie.setErrorContent("邮件保存的路径为：" + StringUtil.saveAsFileWriter(html));
                    bcie.setErrorInfo("请注意,可能是139邮箱的新类型账单，获取账单失败---<江西银行信用卡>");
                }
                saveBillCycleInfoEntity(bcie);
                listbills.add(bcie);
                continue;
            ...
            bcie.setIsSuccess(3);
            bcie.setErrorContent("异常信息为："+e+"。异常邮件保存的路径为："+StringUtil.saveAsFileWriter(html));
